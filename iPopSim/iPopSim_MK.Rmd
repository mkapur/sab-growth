...
for(l in 1:nrow(fLevs)){
  ...
  unlink(out_file, recursive=TRUE) ## delete pre-existing file
  ...
  for(boot_number in 1:nboot) { ## full nsims, line 20
    ...
    Inst_F_vector <- c(select.effort(location = 'inputs/',  level = fLevs[l,1], nsim = 1000)[1:50],select.effort(location = 'inputs/',  level = fLevs[l,1], nsim = 1000)[51:101])
...
for(l in 1:nrow(fLevs)){
  out_file <- paste0(getwd(),"/outputs/",fLevs[l,3],"/",paste(fLevs[l,1],fLevs[l,2], sep = "_"))
  df <- read.csv(paste0(out_file,"/",tolower(paste(fLevs[l,1],fLevs[l,2])),"sim_ageComp.csv"))
}

boots  <- list.dirs(paste0(getwd(),"/outputs/"), recursive = T)[grep('boot',list.dirs(paste0(getwd(),"/outputs/"), recursive = T))]
  ## bind all rows
  dat <- list.files(boots, full.names = T)[grep('SAA',list.files(boots, full.names = T))] %>%
   ...
   dat1 <- dat %>%
    sample_n(10000) %>%
    group_by(Year, Age) %>% summarise(n = n()) %>%
    mutate(freq = n / sum(n))
  dat2 <- dat1 %>% group_by(Year) %>% summarise(totN = sum(n))
