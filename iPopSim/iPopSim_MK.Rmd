...
rm(list = ls())
source('system_presets_raw.R') ## system functions & IBM params
# generate.effort(level = c('LOW','MED','HIGH')[1], nsim = 1000) ## see system_presents for function; only occurs once
nboot <- 3
fLevs <- expand.grid(FLEVEL1 = c('LOW','MED','HIGH'), FLEVEL2 = c('LOW','MED','HIGH'))
...
for(l in 1:nrow(fLevs)){
  out_file <- paste0(getwd(),"/outputs/",paste(fLevs[l,1],fLevs[l,2], sep = "_"))
  ...
  for(boot_number in 1:nboot) { ## full nsims, line 20
    cat(paste0(paste(fLevs[l,1],fLevs[l,2]),' SIM ',boot_number, "\n"))
