...
if(scen != 'tempvar_R1R2'){
  gamREGS0 <-  factor(unique(dat$gamREG), levels=Rlevs)
  gamREGS <- sort(gamREGS0)
} else {
  gamREGS0 <- factor(unique(dat$cREG),levels=Rlevs)
  gamREGS <- sort(gamREGS0)
...
if(is.na(breaksdf$yr_breaks)){
  for(iq in 1:length(gamREGS)){ ## loop regions, this leads to varying # samples (denoms)
    ...
    if(scen == 'NoBreaks' |gamREGS[iq] == 'R4') {cdf[idx,'REG'] <- 'R1'}

    
    
    # if(gamREGS[iq] == 'R3' & is.na(REGS[iq])){
    #   cdf[idx,'REG'] <- 'R2'
    # }
    # if(gamREGS[iq] == 'R4' & is.na(REGS[iq])){
    #   cdf[idx,'REG'] <- 'R1'
    # } 
    
    
    # cdf[idx,'REG'] <- as.factor(if(!is.na(REGS[iq])){REGS[iq]}else{ifelse(gamREGS[iq] == 'R4','R1',last(REGS))}) ## if anything other than R1, will compare to appropriate RX (2 or 3)
    ...
    if(is.na(cdf[idx,'LAT']) |is.na(cdf[idx,'LON']) |is.na(cdf[idx,'YEAR'])) stop(scen," ",b," threw NA comparison")
  ...
  for(t in 1:length(c('early','late'))){
    ...
    for(j in 1:length(unique(tmp0$REG))){ ## loop rows in this period
      ...
      for(iq in 1:length(gamREGS)){ ## loop regions, this leads to varying # samples (denoms)
        ...
        cdf[idx,'boot'] <- b
        # cdf[idx,'REG'] <- as.factor(if(!is.na(REGS[iq])){REGS[iq]}else{last(REGS)})
        cdf[idx,'REG'] <- ifelse(t == 1  | scen == 'NoBreaks', 'R1', ifelse(scen == 'F0L1S_R3','R3','R2')) ## what it was actually compared to
        # if(scen == 'NoBreaks' |gamREGS[iq] == 'R4') {cdf[idx,'REG'] <- 'R1'}
