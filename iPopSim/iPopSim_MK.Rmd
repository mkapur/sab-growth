...
for(l in 1:3){
  ...
  source(paste0('vb_params_1.R')) ## special VB params
  source('system_presets.R') ## system functions 
  
  scenname <- paste0(getwd(),"/",fLevs[l,'DESC'])
  scenID <- ifelse(is.na(fLevs[l,5]), paste(fLevs[l,3],fLevs[l,4],sep = "_"), paste(fLevs[l,3],fLevs[l,4],fLevs[l,5], sep = "_"))
  out_file <-  paste0(scenname,"/",scenID)
  ...
  for(boot_number in 1:nboot) { ## full nsims, line 20
    ...
    Inst_F_vector[1:84] <-   c( ## create final fifty or 34 years
      rep(0, 50),
      generate.effort(level = fLevs[l, "FMORT_1"], nyears = nyears),
      generate.effort(level = fLevs[l, "FMORT_2"], nyears = nyears)
    )
...
for(l in 1:nrow(fLevs)){
  ...
  for(i in 1:nrow(dat)){
    if(is.na(sptl)){
       dat$Latitude_dd[i] <- runif(1, 0.0, 50.0) ## uniform range
