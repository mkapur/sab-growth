...
author: "F Carvalho, update M Kapur 2017"
...
# library(lattice)
rm(list = ls())
source(paste0(getwd(),'/system_presets.R')) ## system functions & IBM params
...
for(level in c('LOW','MED','HIGH')){
  ...
  for(boot_number in 1:nboot) { ## full nsims, line 20
    print(paste0(level,' SIM === ',boot_number))
    ## Designate WRITE location
    path_name = paste0(out_file,"/boot_",boot_number) 
    dir.create(path_name) ## make individual sim file
    ...
    Inst_F_vector = select.effort(location = 'G:/CAPAM/code/inputs/',
                                  level = level, nsim = 1000) ## subset a random time series from the effort simulator for a given fishing level; the 1000 just indicates how many sims it can choose from
    for(ry in 1:simu_year){ ## iterate sim years
      ...
      for(i in 1:length(RECRUITs_year)){   ## iterate through each new recruit
        ...
        # write.table(SAA, paste(path_name,"/IBM_SAA_MATRIX_",level,".txt",sep=""),append = TRUE,row.names = FALSE,col.names = FALSE,sep=",")
        # write.table(dinfo,paste(path_name,"/IBM_DEAD_MATRIX_",level,".txt",sep=""),append = TRUE,row.names = FALSE,col.names = FALSE,sep=",")
