build_simComp <- function(out_file, l=l, dat0 = NA){
  ...
  for(b in 1:length(boots)){
    dat0 <- read.table(paste0(boots[b],"/IBM_SAA_MATRIX.txt"),sep=",",header=T)
    scenID <-
...
plotComps <- function(dat1,dat2,datN,scenID, saveloc = NA,nboots = nboot){
  ...
  for(b in 1:nboot){
    ...
    if(is.na(saveloc)){
      ggsave(p, file = paste0(getwd(),"/plots/",paste0(scenID,b,"_ageComps.jpg")), dpi = 480, height = 10, width = 8, unit = 'in')
    } else{
      ggsave(p, file = paste0(out_file,"/",paste0(scenID,b, "_ageComps.jpg")), dpi = 480, height = 10, width = 8, unit = 'in')
    }
    ...
    if(is.na(saveloc)){
      ggsave(p, file = paste0(getwd(),"/plots/",paste0(scenID,b,"_lenComps.jpg")), dpi = 480, height = 10, width = 8, unit = 'in')
    } else{
      ggsave(p, file = paste0(out_file,"/",paste0(scenID,b,"_lenComps.jpg")), dpi = 480, height = 10, width = 8, unit = 'in')
    }  ## end else
  }## end nboot
