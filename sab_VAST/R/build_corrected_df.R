...
# # species_data <- filter(dataset__, SPECIES_CODE==species_code, 
# #                        Sex==sex)
# species_data <- filter(all_data,  SPECIES_CODE == 20510,
#                        Sex == "F")
# corrected_lengths <- get_unbiased_lengths(
#   species_data,
#   age.name = "Age",
#   length.name = "Length_cm",
#   year.name = "Year",
#   lat.name = "Latitude_dd",
#   lon.name = "Longitude_dd"
# )
# 
# corrected_age<- data.frame(cbind(row.names(corrected_lengths),
#                                  corrected_lengths))
# names(corrected_age) <- c("ID", "lengths")
# corrected_age <- filter(corrected_age, lengths>0)
# species_age = species_data %>% filter(AGE==age) %>%
