...
## load data
load(paste0("./input_data/gam_data_sab_0312.rda")) ## all_data -- made using gam_dataprep and 15k subsample
...
for(t in 1:length(Terms)){
 ...
 plist[[1]] <- ggplot() + 
   ...
   labs(fill = "Length of Age-10 Fish (cm)",size  = "Length of Age-10 Fish (cm)") +
   ggtitle('g) GAM-Estimated Regions, with raw data') +
...
grid.arrange(grobs = plist, layout_matrix = lay)  %>% 
 ggsave(plot = .,  file = paste0("./figures/sab_smooth_map_a10.png"), width = 12, height = 15, units = 'in', dpi = 480)
...
## plot estimates
parest <- read.csv(paste0("./GAM_output/SAB_parEst_gam_2019-03-11.csv")) %>%
...
## plot fits
ypreds <- read.csv( paste0("./GAM_output/SAB_predicts2019-03-11.csv"))
...
plist[[2]] <- ggplot(ypreds[ypreds$Sex == 'F',], aes(x = Age, y = Predicted, col = REG )) +
  ...
  labs(y = 'Length (cm)', col = "Actual Data Source", title = "b)") +
