...
require(tidyr)
require(ggplot2)
# knitr::opts_chunk$set(root.dir =  "C:/Users/Maia Kapur/Dropbox/UW/sab-growth/iPopSim")
rm(list = ls())
# generate.effort(level = c('LOW','MED','HIGH')[1], nsim = 1000) ## see system_presents for function; only occurs once
```
...
options(warn = -1)
nboot <- 4
fLevs <- expand.grid(FLEVEL1 = c('LOW','MED','HIGH'), FLEVEL2 = c('LOW','MED','HIGH'), SPP = c('R1','R2'))
source('wrangle_data.R')
p = proc.time() ## set timer
for(l in 1:nrow(fLevs)){
  source(paste0('vb_params_',fLevs[l,3],'.R')) ## special VB params
  source('system_presets.R') ## system functions 

  out_file <- paste0(getwd(),"/outputs/",fLevs[l,3],"/",paste(fLevs[l,1],fLevs[l,2], sep = "_"))
  ...
  for(boot_number in 1:nboot) { ## full nsims, line 20
    cat(paste0(paste(fLevs[l,1],fLevs[l,2],fLevs[l,3]),' boot ',boot_number, "\n"))
    ...
    Inst_F_vector <- c(rep(0,50),generate.effort(level = fLevs[l,1])[1:24],
                       generate.effort(  level = fLevs[l,1])[24:51])
    
    effDF <- rbind(effDF, cbind(as.numeric(Inst_F_vector),as.numeric(1:51),rep(paste(fLevs[l,1],fLevs[l,2],fLevs[l,3], sep = "_"),51)))

    # Inst_F_vector = select.effort(location = 'inputs/',  level = 'LOW', nsim = 1000) ## subset a random time series from the effort simulator for a given fishing level; the 1000 just indicates how many sims it can choose from
    for(ry in 1:simu_year){ ## iterate sim years
      ...
      for(i in 1:length(RECRUITs_year)){   ## iterate through each new recruit
        ...
        if(length(fish_sizes) >a2){print('length fish size > a2 line 122')}
        ...
        SAA <- data.frame(
          ...
          FSIM = paste(fLevs[l,1],fLevs[l,2]),
          REG = fLevs[l,3]
        )
...
# print(paste0('wrote SS3 mean size at age, sim ',sim_number," FLEVEL ",level))
  ...
  genDat <- build_simComp(out_file)
  plotComps(dat1 = genDat[[1]], dat2 = genDat[[2]], datN = genDat[[3]], saveloc = 1)
  rm(genDat)
} ## end of fishing_level
print(paste("Full simulation time ",((proc.time()-p)/60)[1],"min(s)"))
names(effDF) <- c('F_std','Year','MOD')
effDF$Year <- as.numeric(effDF$Year)
effDF$F_std <- as.numeric(effDF$F_std)
write.csv(effDF,paste0(getwd(),"C:/Users/Maia Kapur/Dropbox/UW/sab-growth/iPopSim/genData/Fsim.csv"),row.names = F )
```


Read in both regions' subsampled data frame from each regime. Then fake lat data with a breakpoint at 14 "degrees". Save this to genData, and plot compiled A-L comp plots to plots/. Now you have a dataset with a temporal break at year 75, and a spatial break at 14 degrees.
```{r stich fake sets, echo = T}
for(l in 1:nrow(fLevs)){
 
  ## get output files for this Ftrend
  lev0 <-  list.dirs(paste0(getwd(),"/outputs/"), full.names = T)[grep(paste(fLevs[l,1],fLevs[l,2],sep = "_"), list.dirs(paste0(getwd(),"/outputs/"), full.names = T))] 
  
  ## get pre-sampled age/length data
  dat <- list.files(lev0, recursive = T, full.names = T)[grep(paste0(tolower(paste(fLevs[l,1],fLevs[l,2])),'sim_Comp.csv'),list.files(lev0, recursive = T, full.names = T))] %>%
    map(read.csv) %>%
  reduce(bind_rows) %>%
  ...
  ## add lat manually
  for(i in 1:nrow(dat)){
    dat$Latitude_dd[i] <- ifelse(dat$REG[i] == 'R1', runif(1,0.0,14.0), runif(1,14.0,28.0))
  }
  
  names(dat)[4] <- "fish_size"
  genDat <- build_simComp(dat0=dat) ## make stuff for plotting
  plotComps(dat1 = genDat[[1]], dat2 = genDat[[2]], datN = genDat[[3]],  saveloc = NA) ## all regions together
  names(dat)[4] <- "Length_cm"
  write.csv(dat, paste0(getwd(),"/genData/",tolower(paste0(unique(dat$FSIM))),"2Regions_Comp.csv"),row.names = F)

  kaputils::plotF(SPRseries = effDF, saveplot = T, pdfcols = 3, plotloc = paste0(getwd(),'/plots') )
}
