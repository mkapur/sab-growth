...
write.csv(rep0, file = paste0("./GAM_output/SAB_parEst_gam_",Sys.Date(),'_phase2.csv'),row.names = F)

ypreds0 <- cbind(dat0,dat) %>% data.frame()  
names(ypreds0)[1] <- c('Predicted')
write.csv(ypreds0,  paste0("./GAM_output/SAB_predicts_",Sys.Date(),"_phase2.csv"),row.names = F)
