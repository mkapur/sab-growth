...
plotComps <- function(dat1,dat2,datN, saveloc = NA){
 ...
 p <- ggplot(dat1, aes(x = Age, y = freq, fill= Flev)) +
  ...
  if(is.na(saveloc)){
    ggsave(p, file = paste0(getwd(),"/plots/",tolower(paste0(fLevs[l,1],fLevs[l,2],"_ageComps.jpg"))), dpi = 480, height = 10, width = 8, unit = 'in')
  } else{
    ggsave(p, file = paste0(out_file,"/",tolower(paste0(fLevs[l,1],fLevs[l,2], "_ageComps.jpg"))), dpi = 480, height = 10, width = 8, unit = 'in')
  }
 ...
 p <-  ggplot(dat2, aes(x = fish_size*10, y = freq, fill= Flev)) +
  ...
  if(is.na(saveloc)){
    ggsave(p, file = paste0(getwd(),"/plots/",tolower(paste0(fLevs[l,1],fLevs[l,2],"_lenComps.jpg"))), dpi = 480, height = 10, width = 8, unit = 'in')
  } else{
    ggsave(p, file = paste0(out_file,"/",tolower(paste0(fLevs[l,1],fLevs[l,2],"_lenComps.jpg"))), dpi = 480, height = 10, width = 8, unit = 'in')
  }  
