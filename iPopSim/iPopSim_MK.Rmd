...
for(l in 1:nrow(fLevs)){
  ...
  for(boot_number in 1:nboot) { ## full nsims, line 20
    ...
    Inst_F_vector <-
      c(rep(0, 50),
      generate.effort(level = fLevs[l, 1], nyears = 25),
      generate.effort(level = fLevs[l, 2], nyears = 26))
    
    fdf <- rbind(fdf,data.frame(cbind(Inst_F_vector[51:101],1:51, rep(idx,51))))
...
## Plot F trajectory once time
  kaputils::plotF(SPRseries = fdf, saveplot = T, pdfcols = 3, plotloc = paste0(getwd(),'/plots') )
