...
scenname <- paste0(getwd(),"/",fLevs[l,'DESC'])
  ...
  if(fLevs[l,'CAT'] %in% c('NONE','SPATIAL')){ ## if spatial only, aggregate all and split regionally
    ...
    dat <- list.files(scenname, recursive = T, full.names = T)[grep('*sim_Comp.csv',list.files(scenname, recursive = T, full.names = T))] %>%
      map(read.csv) %>%
      reduce(bind_rows) %>%
  ...
  write.csv(dat, paste0(getwd(),"/genData/",basename(scenname),"_",scenID,".csv"),row.names = F)
  
}

## Plot F trajectory one time
fdf %>% filter(MOD %in% floor(runif(6,1,35)) & boot == 1) %>% 
